...
def execute_resumable_upload(request, output_name: str) -> dict:
    response = None
    while response is None:
        status, response = request.next_chunk()
        if status:
            print(f"Upload progress for {output_name}: {int(status.progress() * 100)}%")
    return response


def upload_output_once(
    service,
    source_path: Path,
    output_name: str,
    parent_id: str,
    ...
    if existing_file:
        ...
        return execute_resumable_upload(request, output_name)
...
def upload_output(
    ...
    for attempt in range(max_retries + 1):
        try:
            return upload_output_once(service, source_path, output_name, parent_id, chunk_size)
        except ResumableUploadError as error:
            if attempt >= max_retries:
                raise
            delay = next_retry_delay(attempt)
            print(
                "ResumableUploadError during Google Drive upload "
                f"(attempt {attempt + 1}/{max_retries + 1}); retrying in {delay}s: {error}"
            )
            time.sleep(delay)
        except HttpError as error:
            if attempt >= max_retries or not is_retryable_http_error(error):
                raise
            delay = next_retry_delay(attempt)
            print(
                "Retryable Google Drive HTTP error during upload "
                f"(attempt {attempt + 1}/{max_retries + 1}); retrying in {delay}s: {error}"
            )
            time.sleep(delay)

    raise RuntimeError("Google Driveアップロードのリトライ上限に到達しました。")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Resumable uploadのチャンクサイズ（bytes）",
    )
    parser.add_argument(
        "--max-retries",
        type=int,
        default=DEFAULT_MAX_RETRIES,
        help="ResumableUploadError発生時の最大リトライ回数",
    )
