...
The PR-only `street-thirds-test.json` reuses the previously marked left white window on the street-night source (same source hash), with a 9%-width thirds tolerance. It does not change the approved street profile or production matching. The street matrix job checks frames 0, 285 and 899 against the unchanged source, rejects dimming and distant changes, then uploads `street-thirds-compare-30s.mp4` alongside the existing comparison. This is a provisional visual sample: the owner must inspect whether that specific window is the desired focal point and the glow looks natural before merging or using it for production.
