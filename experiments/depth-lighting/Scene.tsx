...
export type LightingProfile = typeof approvedProfile & {pairs?: LightPair[]; timing?: {windows: Record<Depth, [number, number][]>; fadeFrames: number}};
...
export const Scene: React.FC<{baseline?: boolean; profile?: LightingProfile}> = ({baseline = false, profile = approvedProfile as LightingProfile}) => {
  ...
  const units: LightPair[] = profile.pairs ?? layers.map(depth => ({id:depth,depth,sourceMask:profile.layers[depth],reflectionMask:'',reflectionGain:0}));
