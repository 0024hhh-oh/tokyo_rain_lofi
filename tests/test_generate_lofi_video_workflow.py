...
def test_workflow_prefers_debug_drive_folder_id_then_incoming_folder_id():
    text = workflow_text()

    assert "WORKFLOW_DISPATCH_DRIVE_FOLDER_ID: ${{ inputs.DRIVE_FOLDER_ID || '' }}" in text
    assert "DRIVE_FOLDER_ID: ${{ inputs.DRIVE_FOLDER_ID || steps.incoming.outputs.work_folder_id }}" in text
...
def test_workflow_logs_debug_inputs_and_selected_acquisition_method_before_download():
    ...
    log_input = 'echo "workflow_dispatch debug inputs: DRIVE_FOLDER_ID=${WORKFLOW_DISPATCH_DRIVE_FOLDER_ID} video_number=${WORKFLOW_DISPATCH_VIDEO_NUMBER}"'
